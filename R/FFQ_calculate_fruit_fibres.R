...
#'
#' @return Calculate the fibre consumptions from fruit out of the FFQ object.
#'
...
FFQ.calculate.fruit.fibres <- function (FFQobject, ignore.groups = FALSE){
  transformationdatafruit <- FFQobject@references$fruit
  ...
  fruitfibrelist <- list(); fruittiminglist <- list(); person_list <- list()
  ...
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Fruit" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
  ...
  rownames(df_diet_d1_3) <- df_diet_d1_3$Group
  ...
  df_diet_d2 <- subset(df_diet_d2, Subcategory == "Fruit" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
                         Group != "Frequency" & Group != "Meal_included" & Group == "Other")
  ...
  df_diet_d2_4 <- subset(df_diet_d2, df_diet_d2$combined == "Source_")
  ...
  ## Name rownames according to new group
  ...
  if(ignore.groups == FALSE){
    x <- data.frame(Check = df_diet_d$Group %in% transformationdatafruit$Group, Group = df_diet_d$Group)
    x <- subset(x, Check == FALSE)
    if(length(unique(x$Check)) > 0){stop(message("Woops!"), paste0("The following groups are not included in the reference list: ", x$Group, "\nYou can add fruits to the list with the FFQ.add.fruit() function"))}
  }
  ## Join with diet data
  df_diet_d <- left_join(df_diet_d, transformationdatafruit, by = "Group")
  ##df_diet_d <- merge(x=df_diet_d, y=transformationdata, by = "Group", all.x=TRUE)
  ...
  assign(paste0("functdata_fruit_person_", i), df_diet_d)
  ...
  assign(paste0("functdata_fruit_fibre_", i), df_diet_d1)
  fruitfibrelist[[i]] <- df_diet_d1
  # Extract timing
  df_diet_d1 <- subset.data.frame(functdata_diet, select = c(1,2,3,4,5,6,6+i))
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Fruit")
  ...
  fruittiminglist[[i]] <- df_diet_d1
  FFQobject@total.fibres.per.group <- list(fruit = fruitfibrelist[[i]])
  FFQobject@group.timing <- list(fruit = fruittiminglist[[i]])
  FFQobject@fibres.per.item <- list(fruit = df_diet_d)
  message("Fruits: done")
  return(FFQobject)
}
