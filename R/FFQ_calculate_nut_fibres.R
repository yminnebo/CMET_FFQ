#' Calculate the fibre consumptions of nuts
#'
#' @return Calculate the fibre consumptions from nuts out of the FFQ object.
#'
...
#' @export
#' @examples
#' FFQobject_nut_fibres <- FFQ.calculate.nut.fibres(FFQobject)



FFQ.calculate.nut.fibres <- function (FFQobject, ignore.groups = FALSE){
  transformationdatanut <- FFQobject@references$nut
  ...
  person_list <- list(); nutfibrelist <- list(); nuttiminglist <- list()
  ...
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Nuts" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
  ...
  df_diet_d2 <- subset(df_diet_d2, Subcategory == "Nuts" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
                         Group != "Frequency" & Group != "Meal_included" & Group == "Other")
  ...
  df_diet_d2 <- as.data.frame(cbind(Category = df_diet_d2_3$Category,
                                    Subcategory = df_diet_d2_3$Subcategory,
                                    Group = df_diet_d2_4[,7],
                                    Subgroup = df_diet_d2_3$Subgroup,
                                    Base_unit = df_diet_d2_3$Base_unit,
  ...
  if(ignore.groups == FALSE){
    x <- data.frame(Check = df_diet_d$Group %in% transformationdatanut$Group, Group = df_diet_d$Group)
    x <- subset(x, Check == FALSE)
    if(length(unique(x$Check)) > 0){stop(message("Woops!"), paste0("The following groups are not included in the reference list: ", x$Group, "\nYou can add nuts to the list with the FFQ.add.nut() function"))}
  }
  ## Join with diet data
  df_diet_d <- left_join(df_diet_d,transformationdatanut, by = "Group")
  ...
  assign(paste0("functdata_nut_person_", i), df_diet_d)
  ...
  assign(paste0("functdata_nut_fibre_", i), df_diet_d1)
  nutfibrelist[[i]] <- df_diet_d1
  # Extract timing
  df_diet_d1 <- subset.data.frame(functdata_diet, select = c(1,2,3,4,5,6,6+i))
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Nuts")
  ...
  nuttiminglist[[i]] <- df_diet_d1
  FFQobject@total.fibres.per.group <- c(FFQobject@total.fibres.per.group, list(nut = nutfibrelist[[i]]))
  FFQobject@group.timing <- c(FFQobject@group.timing, list(nut = nuttiminglist[[i]]))
  FFQobject@fibres.per.item <- c(FFQobject@fibres.per.item, list(nut = df_diet_d))
  message("Nuts: done")
  return(FFQobject)
}
