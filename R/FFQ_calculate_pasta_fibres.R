#' Calculate the fibre consumptions of pasta
#'
#' @return Calculate the fibre consumptions from pasta out of the FFQ object.
#'
...
#' @export
#' @examples
#' FFQobject_pasta_fibres <- FFQ.calculate.pasta.fibres(FFQobject)


FFQ.calculate.pasta.fibres <- function (FFQobject, ignore.groups = FALSE){
  ...
  person_list <- list(); pastafibrelist <- list(); pastatiminglist <- list()
  i=1
  ...
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Pasta" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
  ...
  df_diet_d2 <- subset(df_diet_d2, Subcategory == "Pasta" &  #WEGLATEN WANNEER WE MEER DAN FRUIT HEBBEN
  ...
  assign(paste0("functdata_pasta_person_", i), df_diet_d)
  ...
  assign(paste0("functdata_pasta_fibre_", i), df_diet_d1)
  pastafibrelist[[i]] <- df_diet_d1
  # Extract timing
  df_diet_d1 <- subset.data.frame(functdata_diet, select = c(1,2,3,4,5,6,6+i))
  df_diet_d1 <- subset(df_diet_d1, Subcategory == "Pasta")
  ...
  pastatiminglist[[i]] <- df_diet_d1
  FFQobject@total.fibres.per.group <- c(FFQobject@total.fibres.per.group, list(pasta = pastafibrelist[[i]]))
  FFQobject@group.timing <- c(FFQobject@group.timing, list(pasta  = pastatiminglist[[i]]))
  FFQobject@fibres.per.item <- c(FFQobject@fibres.per.item, list(pasta  = df_diet_d))
  message("Pasta: done")
  return(FFQobject)
}
